...
pub struct Editor {
    ...
    pub my_scroll_manager: Entity<ScrollManager>,
    // 
    pub primary_scroll_manager: Option<Entity<ScrollManager>>,
...
impl Editor {
    ...
    pub fn clone(&self, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        clone.scroll_manager.update(cx, |scroll_manager, cx| {
            scroll_manager.clone_state(self.scroll_manager.read(cx))
        });
    ...
    fn new_internal(
        ...
        cx.observe(&scroll_manager, |editor, _, cx| {
            cx.notify();
        })
        .detach();
        ...
        editor.scroll_manager.update(cx, |scroll_manager, cx| {
            scroll_manager.show_scrollbars(window, cx);
        });
    ...
    pub fn set_scroll_manager(&mut self, scroll_manager: Entity<ScrollManager>) {
    ...
    pub fn text_layout_details(&self, window: &mut Window, cx: &App) -> TextLayoutDetails {
        TextLayoutDetails {
            ...
            scroll_anchor: self.scroll_manager.read(cx).anchor(),
            visible_rows: self.visible_line_count(cx),
            vertical_scroll_margin: self.scroll_manager.read(cx).vertical_scroll_margin,
        }
    }
    ...
    fn push_to_nav_history(
        ...
        if let Some(nav_history) = self.nav_history.as_mut() {
            ...
            let scroll_state = self.scroll_manager.read(cx).anchor();
    ...
    fn initialize_new_minimap(
        ...
        minimap.scroll_manager.update(cx, |scroll_manager, cx| {
            scroll_manager.clone_state(self.scroll_manager.read(cx));
        });
